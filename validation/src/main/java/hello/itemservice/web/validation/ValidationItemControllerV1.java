...
public class ValidationItemControllerV1 {
    ...
    public String addItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (item.getItemName() == null || !StringUtils.hasText(item.getItemName())) {
            ...
            log.info("getItemName errors={}", errors);
        }

        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            errors.put("price", "가격은 1,000원 ~ 1,000,000 까지 허용합니다.");
            log.info("getPrice errors={}", errors);
        }

        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
            errors.put("quantity", "수량은 최대 9,999 까지 허용합니다.");
            log.info("getQuantity errors={}", errors);
        }
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10000) {
                ...
                log.info("resultPrice errors={}", errors);
            }

        }
        if (!errors.isEmpty()) {
            log.info("isEmpty errors={}", errors);
